		...
		while (true) {// 정수를 다시 받기위해 while문을 썻다.
			...
			if (userNum % 2 == 1 && userNum >= 3) { // 입력값은 홀수 이면서 3이상의 값만 남긴채 진행된다.
				...
				for (int i = 0; i < userNumArr.length; i++) {
					...
					if (i < userNumArr.length / 2) {
						userNumArr[i] = num++; // 배열에 값 대입
					} else {
						userNumArr[i] = num--;
					}
	...
	public void practice16() {
		...
		while (true) {
			...
			if (ans == 'y' || ans == 'Y') {
				...
				int userWantsMore = sc.nextInt();
				sc.nextLine();
				String[] copyArr = Arrays.copyOf(userSize, userSize.length + userWantsMore - 1);
				for (int i = count; i < count + userWantsMore; i++) {
					
					if (count == copyArr.length) {
						break;
					} else {

						System.out.print((count + 2) + "번째 문자열 : ");
						String arrsize = sc.nextLine();
						copyArr[i] = arrsize;
						count+=1;;
					}
